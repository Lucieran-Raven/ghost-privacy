...
export function cleanupOldNonces(
  ...
  // Also check if we need to evict due to size limits
  const MAX_NONCE_ENTRIES = 5000;
  const needsSizeCleanup = state.nonceMetadata.size > MAX_NONCE_ENTRIES;

  if (!hasExpired && !needsSizeCleanup) {
    return state;
  }
  ...
  // First: remove expired nonces by age
  if (hasExpired) {
  ...
  // Second: if still over limit, remove oldest by received time (LRU eviction)
  if (nonceMetadata.size > MAX_NONCE_ENTRIES) {
    const entries = Array.from(nonceMetadata.entries())
      .sort((a, b) => a[1].receivedAt - b[1].receivedAt);
    const toDelete = entries.slice(0, entries.length - MAX_NONCE_ENTRIES);
