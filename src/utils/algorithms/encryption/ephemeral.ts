...
export async function generateEcdhKeyPair(deps: EphemeralCryptoDeps): Promise<CryptoKeyPair> {
  // Generate keypair with temporary extractability to convert to non-extractable
  const keyPair = await deps.subtle.generateKey(
    { name: 'ECDH', namedCurve: 'P-256' },
    true,
    ['deriveKey', 'deriveBits']
  );
  
  // Export and immediately re-import as non-extractable to minimize exposure window
  let privateKeyPkcs8: ArrayBuffer | null = await deps.subtle.exportKey('pkcs8', keyPair.privateKey);
  ...
  try {
    ...
    // Immediately zeroize the exported key material
    const wipeView = new Uint8Array(privateKeyPkcs8);
    deps.getRandomValues(wipeView);
    wipeView.fill(0);
    privateKeyPkcs8 = null;
    
    return { publicKey: keyPair.publicKey, privateKey };
  } catch (error) {
    // Ensure cleanup on error path too
    if (privateKeyPkcs8) {
      try {
        const wipeView = new Uint8Array(privateKeyPkcs8);
        deps.getRandomValues(wipeView);
