...
export class MultiDeviceSync {
  ...
  private async generateX25519Keypair(): Promise<{publicKey: Uint8Array; secretKey: Uint8Array}> {
    ...
    // Derive public key (simplified - would use actual X25519)
    const publicKey = await this.x25519PublicFromSecret(secretKey);
  ...
  private async x25519SharedSecret(
    ...
    // Simplified DH - actual implementation uses scalar multiplication on curve25519
