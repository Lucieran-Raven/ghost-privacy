...
export class MoneroPaymentManager {
  ...
  private async deriveSubaddress(index: number): Promise<string> {
    ...
    // Formula: subaddress = H("SubAddr" || view_key || index) * G + primary_address
    
    const prefix = new TextEncoder().encode('SubAddr');
    const viewKeyBytes = Buffer.from(this.wallet.viewKey, 'hex');
    const indexBytes = new Uint8Array(4);
    new DataView(indexBytes.buffer).setUint32(0, index, false);
    
    const input = Buffer.concat([prefix, viewKeyBytes, indexBytes]);
    
    // Hash to scalar
    const scalar = await crypto.subtle.digest('SHA-256', input);
    
    // In production: Multiply by curve generator point
    // Simplified: return formatted address
    const hash = Array.from(new Uint8Array(scalar))
      .map(b => b.toString(16).padStart(2, '0'))
      .join('');
    
    // Generate subaddress from hash (simplified)
  ...
  private formatMoneroAddress(publicSpendKey: string): string {
    ...
    // Network byte (0x12 for mainnet subaddress) || public spend key (32 bytes) || public view key (32 bytes) || checksum
    
    const networkByte = '12'; // Subaddress network byte
    const viewKey = this.wallet!.viewKey.slice(0, 64);
    
    const rawAddress = networkByte + publicSpendKey + viewKey;
    
    // Calculate checksum (first 4 bytes of keccak-256)
    // Simplified - actual checksum calculation
    const checksum = rawAddress.slice(0, 8);
    
    // Base58 encode
    return this.base58Encode(Buffer.from(rawAddress + checksum, 'hex'));
  }
