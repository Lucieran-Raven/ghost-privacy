...
const SAFE_COLOR_PATTERN = /^(#[0-9A-Fa-f]{3,8}|rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)|rgba\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*[\d.]+\s*\)|hsl\(\s*\d{1,3}\s*,\s*[\d.]+%?\s*,\s*[\d.]+%?\s*\)|hsla\(\s*\d{1,3}\s*,\s*[\d.]+%?\s*,\s*[\d.]+%?\s*,\s*[\d.]+\s*\)|[a-zA-Z]+)$/;

const sanitizeColor = (color: string | undefined): string | null => {
  if (!color) return null;
  const trimmed = color.trim();
  // Strict validation: only allow valid CSS color formats
  if (!SAFE_COLOR_PATTERN.test(trimmed)) {
    return null;
  }
  // Additional check: no semicolons, braces, or other injection vectors
  if (/[;{}()<>]/.test(trimmed) && !trimmed.startsWith('rgb') && !trimmed.startsWith('hsl')) {
    return null;
  }
  return trimmed;
};
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Build CSS using validated values and React's style element (no dangerouslySetInnerHTML)
